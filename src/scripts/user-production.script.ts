...
async function bootstrap() {
   ...
   */
  const aggregateRewards = (rewards: CloudKReward[]): any => {
    return rewards.reduce((acc, reward) => {
      const dateKey = new Date(reward.forDate).toISOString().split('T')[0]; // Group by date (YYYY-MM-DD)
      ...
      if (!acc[userKey][dateKey]) {
        acc[userKey][dateKey] = { myProduction: reward.tokenAmount };
      }
  ...
  while (hasMoreData) {
    ...
    const rewards = await CloudKRewards.find({ deletedAt: null })
      .skip(skip)
      .limit(10);
      ...
      const promises = chunk.map(async ([user, dates]) => {
        ...
        const datePromises = Object.entries(dates).map(async ([date, data]) => {
          ...
          try {
            ...
            const myProduction: number = (data as DateRewardData).myProduction;

            // Create or update rewards for the user
            await cloudKRewardService.createOrUpdateRewards(
              new Types.ObjectId(user),
              myProduction,
              date, // Ensure the exact target date is passed
            );
