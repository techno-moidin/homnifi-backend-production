...
import { DueReferenceMetaData } from '@/src/wallet/interfaces/withdraw-transaction.interface';
...
async function bootstrap() {
  ...
  try {
    ...
    const lykToken = await tokenModel.findOne({
      symbol: 'lyk-w',
      deletedAt: null,
    });

    if (!lykToken) {
      console.error('lykToken token not found');
      return;
    }
    ...
    const findMissedTransaction: any = [];
    for (
      let index = 0;
      index <
          ...
          if (
            ...
            if (findDebitTransaction === element.amount) {
              ...
              if (advanceValidation) {
                ...
                const dueMeta: DueReferenceMetaData = {
                  fromAmount: findNextTransaction.total,
                  fromToken: findNextTransaction.token,
                  fromWallet: findNextTransaction.fromWallet,
                  fromWalletTrx: findNextTransaction.fromWalletTrx,
                  deductedAmount: element.amount,
                  amount: element.amount,
                  type: DueType.WITHDRAW,
                  DueRemark: DueRemarks.FULL_DEBIT_WITHDRAW,

                  tokenPrice: element.tokenPrice,
                  duewalletId: element.fromWallet,
                  dueWalletTransactionId: element.fromWalletTrx,
                  withdrawTransactionId: findNextTransaction._id,
                  depositTransactionId: null,
                  cloudkTransactionId: null,
                  snBonusTransactionId: null,
                  remark: element.userRemarks,
                  note: element.note,
                  isReverted: true,
                  revertedWalletTransactionId: null,
                };

                const updateTransactiondue =
                  await withdrawTransactionModel.findByIdAndUpdate(
                    element._id,
                    {
                      dueWithdrawReference: null,
                      dueRemarks: null,
                      meta: dueMeta,
                    },
                  );
                if (updateTransactiondue) {
                  console.log(
                    `Transaction ${element._id} DUE updated successfully`,
                  );
                }
                ...
                if (updateTransaction) {
                  console.log(
                    `Transaction ${element._id} NEXT updated successfully`,
                  );
                }
              }
            } else {
              ...
              if (advanceValidation) {
                ...
                const dueMeta: DueReferenceMetaData = {
                  ...
                  deductedAmount: element.amount,
                  amount: element.amount,
                  DueRemark: DueRemarks.PARTIAL_DEBIT_WITHDRAW,
                  type: DueType.WITHDRAW,
                  tokenPrice: element.tokenPrice,
                  duewalletId: element.fromWallet,
                  dueWalletTransactionId: element.fromWalletTrx,
              ...
              if (!advanceValidation && !findPartialDebitTransaction) {
                element.specialType =
                  element.specialType + ` without connection`;
                element.duetype = findNextTransaction.type;

                const dueMeta: DueReferenceMetaData = {
                  fromAmount: element.total / element.tokenPrice,
                  fromToken: lykToken._id as Types.ObjectId,
                  fromWallet: null,
                  fromWalletTrx: null,
                  deductedAmount: element.amount,
                  amount: element.amount,
                  DueRemark: DueRemarks.NodeK_CLAIMED_FULL_DEBIT_WITHDRAW,
                  type: DueType.NODEK,
                  tokenPrice: element.tokenPrice,
                  duewalletId: element.fromWallet,
                  dueWalletTransactionId: element.fromWalletTrx,
                ...
                findMissedTransaction.push(element);
              }
            }
          } else {
    ...
    for (let index = 0; index < findMissedTransaction.length; index++) {
      const element = findMissedTransaction[index];
      console.log('Missed Transaction : ', element, '-');
    }
    console.log('Missed Transaction : ', findMissedTransaction.length, '-');
